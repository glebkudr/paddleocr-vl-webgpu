//! Structural contract for the sealed M6e4 persistent browser decoder
//! attention-block session module before any production exists.
...
const CAUSAL_MODULE: &str = "crates/pvlc-runtime-web/src/web/decoder_layer_session.rs";
const AUTHORITY: &str = "DecoderLayerSessionAuthority";
const SESSION: &str = "BrowserDecoderLayerSession";
const OWNER_FIELD: &str = "decoder_layer_session";

#[test]
fn decoder_layer_session_is_one_cfg_free_sealed_causal_module() {
    ...
    assert_eq!(
        declarations.len(),
        1,
        "Web runtime must declare decoder_layer_session exactly once"
    );
    ...
    assert!(
        ...
        "decoder_layer_session must be one unconditional private out-of-line module"
    );
    ...
    assert_eq!(
        authorities.len(),
        1,
        "sealed DecoderLayerSessionAuthority must be declared exactly once"
    );
    ...
    assert!(
        ...
        "sealed authority must privately and directly own exactly crate::AsyncSessionOwner<BrowserDecoderLayerSession> last"
    );
    ...
    assert_eq!(
        sessions.len(),
        1,
        "private BrowserDecoderLayerSession must be declared exactly once"
    );
    ...
    assert!(
        ...
        "browser decoder layer session must be unconditional, private, and non-generic"
    );
    let mut field_class_counts = [0usize; 6];
    for field in &session.fields {
        assert!(
            inherited(&field.vis) && only_doc_attributes(&field.attrs),
            "browser decoder layer session fields must all be private"
        );
        let Some(class) = classify_session_field(&field.ty) else {
            panic!(
                "browser decoder layer session field hides state outside the closed persistent algebra"
            )
        };
    ...
    assert_eq!(
        ...
        29,
        "browser decoder layer session must directly own its twenty-nine exact wgpu::webgpu::GpuBuffer resources"
    );
    assert_eq!(
        field_class_counts[SessionFieldClass::JsObjectHandle as usize],
        15,
        "browser decoder layer session must directly own its six pipeline and nine bind-group handles"
    );
    assert_eq!(
        field_class_counts[SessionFieldClass::CoreKvPlan as usize],
        1,
        "browser decoder layer session must hold exactly one exact core DecoderKvSessionPlan"
    );
    assert_eq!(
        field_class_counts[SessionFieldClass::CoreAttentionPlan as usize],
        1,
        "browser decoder layer session must hold exactly one exact core DecoderAttentionBlockPlan"
    );
    assert_eq!(
        field_class_counts[SessionFieldClass::Scalar as usize],
        3,
        "browser decoder layer session must keep exactly its cache position and poison/ready state in plain scalars"
    );
    assert_eq!(
        field_class_counts[SessionFieldClass::Digest as usize],
        6,
        "browser decoder layer session must keep exactly the six canonical shader digests"
    );
    ...
    assert_eq!(
        session_impls.len(),
        1,
        "browser decoder layer session must have one inherent implementation"
    );
    ...
    assert!(
        ...
        "browser decoder layer session implementation must be unconditional and inherent"
    );
    ...
    assert!(
        ...
        "sealed begin must bind the exact descriptor.plan() results of both the cache and the attention block and feed them to the live session constructor"
    );
    assert!(
        planner_result_reaches_live_consumer(step, "plan_step", 1, "owner"),
        "sealed step must bind the exact attention-block plan_step() result and feed it to the live step executor"
    );
    assert!(
...
#[test]
fn web_runtime_has_one_closed_layer_owner_and_direct_exact_wasm_delegations() {
    let root = parse(WEB_RS);
    ...
    assert!(
        matching_impls(&first_effect, "WebRuntime").is_empty()
            && matching_impls(&first_effect, AUTHORITY).is_empty(),
        "the existing first-effect module must not become a WebRuntime or decoder-layer-authority side door"
    );
    let kv_module = parse("crates/pvlc-runtime-web/src/web/decoder_kv_session.rs");
    assert!(
        matching_impls(&kv_module, AUTHORITY).is_empty()
            && matching_impls(&kv_module, SESSION).is_empty(),
        "the M6e3 KV module must not become a decoder-layer-authority side door"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        "web.rs must not extend the sealed decoder layer authority"
    );
    for alias in root.items.iter().filter_map(|item| match item {
        ...
        assert!(
            ...
            "web.rs must not hide the decoder layer authority behind a type alias"
        );
    }
    ...
    assert_eq!(
        owner_fields.len(),
        1,
        "WebRuntime must own exactly one DecoderLayerSessionAuthority"
    );
    ...
    assert!(
        inherited(&owner_field.vis)
            ...
            && only_doc_attributes(&owner_field.attrs),
        "WebRuntime decoder layer authority must be one unconditional private named field"
    );

    let expected_delegations = [
        ("abort_decoder_layer_session", "abort"),
        ("begin_decoder_layer_session", "begin"),
        (
            "begin_decoder_layer_session_with_shader_override",
            "begin_with_shader_override",
        ),
        ...
        ("finish_decoder_layer_session", "finish"),
        ("step_decoder_layer_session", "step"),
    ];
    let wasm_impls = root
        .items
        .iter()
        ...
        .filter_map(|item| match item {
            ImplItem::Fn(function)
                if function
                    ...
                    .ident
                    .to_string()
                    .contains("decoder_layer_session") =>
            {
                Some(function.sig.ident.to_string())
            }
            _ => None,
    ...
    assert_eq!(
        ...
        expected_delegations
            ...
            .collect(),
        "decoder layer WASM operation allowlist drifted"
    );
...
fn sealing_helpers_require_direct_owner_and_allow_only_documentation_attributes() {
    let direct = syn::parse_str::<Type>("crate::AsyncSessionOwner<BrowserDecoderLayerSession>")
        .expect("direct owner type must parse");
    let unqualified = syn::parse_str::<Type>("AsyncSessionOwner<BrowserDecoderLayerSession>")
        .expect("unqualified owner type must parse");
    let decoy = syn::parse_str::<Type>("decoy::AsyncSessionOwner<BrowserDecoderLayerSession>")
        .expect("decoy owner type must parse");
    let wrapped = syn::parse_str::<Type>("RefCell<AsyncSessionOwner<BrowserDecoderLayerSession>>")
        .expect("wrapped owner type must parse");
    let tuple =
        syn::parse_str::<Type>("(AsyncSessionOwner<BrowserDecoderLayerSession>, ExtraAuthority)")
...
fn delegation_helper_requires_exact_argument_forwarding() {
    let file = syn::parse_file(
        r#"
        impl WebRuntime {
            pub fn step_decoder_layer_session(&self, hidden: Bytes) -> Result {
                self.decoder_layer_session.step(hidden)
            }
            pub fn swapped(&self, hidden: Bytes, extra: Bytes) -> Result {
                self.decoder_layer_session.step(extra)
            }
        }
        "#,
    )
...
fn planner_results_must_reach_live_consumers() {
    let live_begin = parse_impl_method(
        ...
        impl DecoderLayerSessionAuthority {
            fn begin(&self, descriptor: Descriptor) -> Result<String, Error> {
                ...
                let attention_plan = attention_descriptor.plan()?;
                let session = BrowserDecoderLayerSession::create(&self.device, &self.queue, kv_plan, attention_plan)?;
    ...
    let live_step = parse_impl_method(
        ...
        impl DecoderLayerSessionAuthority {
            fn step(&self, hidden: Bytes) -> Result<String, Error> {
                ...
                let step_plan = session.attention_plan.plan_step(transition.cache_tokens_before, &hidden)?;
    ...
    let ignored_second_plan = parse_impl_method(
        ...
        impl DecoderLayerSessionAuthority {
            fn begin(&self, descriptor: Descriptor) -> Result<String, Error> {
                let kv_plan = kv_descriptor.plan()?;
                let attention_plan = attention_descriptor.plan()?;
                let session = BrowserDecoderLayerSession::create(&self.device, &self.queue, kv_plan)?;
                let generation = self.owner.begin(session)?;
                Ok(generation.to_string())
            }
        }
    ...
    let duplicated_call = parse_impl_method(
        ...
        impl DecoderLayerSessionAuthority {
            fn begin(&self, descriptor: Descriptor) -> Result<String, Error> {
                ...
                let attention_plan = attention_descriptor.plan()?;
                let session = BrowserDecoderLayerSession::create(&self.device, &self.queue, second, attention_plan)?;
                let generation = self.owner.begin(session)?;
                Ok(generation.to_string())
            }
        }
    ...
    let transitive_shadow = parse_impl_method(
        ...
        impl DecoderLayerSessionAuthority {
            fn begin(&self, descriptor: Descriptor) -> Result<String, Error> {
                let kv_plan = kv_descriptor.plan()?;
                let attention_plan = attention_descriptor.plan()?;
                let shadow = attention_plan;
                let session = BrowserDecoderLayerSession::create(&self.device, &self.queue, kv_plan, shadow)?;
                let generation = self.owner.begin(session)?;
                Ok(generation.to_string())
            }
        }
    ...
    let ignored_step_plan = parse_impl_method(
        ...
        impl DecoderLayerSessionAuthority {
            fn step(&self, hidden: Bytes) -> Result<String, Error> {
                let (lease, mut session) = self.owner.acquire()?;
                let transition = session.kv_plan.plan_cache_transition(session.cache_tokens)?;
                let step_plan = session.attention_plan.plan_step(session.cache_tokens, &hidden)?;
                session.cache_tokens = transition.cache_tokens_after;
                let outcome = self.owner.complete(lease, session, CompletionAction::Restore);
                Ok(format!("{outcome:?}"))
            }
        ...
        impl DecoderLayerSessionAuthority {
            fn begin(&self, descriptor: Descriptor) -> Result<Promise, Error> {
                let prepared = (|| {
                    let kv_plan = kv_descriptor.plan()?;
                    let attention_plan = attention_descriptor.plan()?;
                    Ok((kv_plan, attention_plan, sources()))
                })();
                let (kv_plan, attention_plan, sources) = match prepared {
                    Ok(prepared) => prepared,
                    Err(error) => return reject(error),
                };
                future_to_promise(async move {
                    run_begin(kv_plan, attention_plan, sources).await
                })
            }
        }
        "#,
    ...
    let forged_plans: File = syn::parse_file(
        ...
        fn forge() {
            ...
            let _ = pvlc_runtime_core::DecoderAttentionBlockPlan {};
            let _ = pvlc_runtime_core::DecoderAttentionBlockStepPlan {};
        }
        "#,
    )
...
fn session_field_algebra_rejects_host_shadows_and_custom_wrappers() {
    ...
    for source in [
        ...
        "wgpu::Device",
        "wgpu::Queue",
        "crate::AsyncSessionOwner<BrowserDecoderLayerSession>",
        ...
        "Option<pvlc_runtime_core::DecoderAttentionBlockPlan>",
        "pvlc_runtime_core::DecoderKvSessionStepPlan",
        "pvlc_runtime_core::DecoderAttentionBlockStepPlan",
        "pvlc_runtime_core::DecoderAttentionBlockStep<'_>",
        "crate::DecoderKvSessionPlan",
        "super::DecoderKvSessionPlan",
        "crate::DecoderAttentionBlockPlan",
        "super::DecoderAttentionBlockPlan",
    ...
    assert_eq!(counts[SessionFieldClass::WgpuWebGpuBuffer as usize], 29);
    assert_eq!(counts[SessionFieldClass::JsObjectHandle as usize], 15);
    assert_eq!(counts[SessionFieldClass::CoreKvPlan as usize], 1);
    assert_eq!(counts[SessionFieldClass::CoreAttentionPlan as usize], 1);
    assert_eq!(counts[SessionFieldClass::Scalar as usize], 3);
    assert_eq!(counts[SessionFieldClass::Digest as usize], 6);
...
#[test]
fn decoder_layer_authority_has_no_crate_wide_side_doors() {
    ...
    for file in files {
        ...
        for alias in &aliases.0 {
            ...
            assert!(
                ...
                "{} hides the sealed decoder layer session behind a type alias",
                relative.to_string_lossy()
            );
        }
    }
