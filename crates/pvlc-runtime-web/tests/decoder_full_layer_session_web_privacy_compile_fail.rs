...
#[test]
fn decoder_full_layer_authority_field_is_present_but_inaccessible_outside_the_crate() {
    let fixture_root =
        workspace_path("target/m6e5-decoder-full-layer-authority-compile-fail/fixture");
    let fixture_src = fixture_root.join("src");
    let target_dir = workspace_path("target/m6e5-decoder-full-layer-authority-compile-fail/build");
    ...
    fs::copy(
        workspace_path(
            "crates/pvlc-runtime-web/tests/compile_fail/full_layer_authority_privacy.rs",
        ),
    ...
    fs::write(
        ...
        format!(
            r#"[package]
name = "pvlc-decoder-full-layer-authority-privacy-probe"
version = "0.0.0"
edition = "2024"
publish = false
...
"#,
    ...
    assert!(
        ...
        "external crate unexpectedly accessed WebRuntime decoder full layer authority"
    );
    let stderr = String::from_utf8(output.stderr).expect("cargo diagnostics must be UTF-8");
    assert!(
        stderr.contains("error[E0616]")
            && stderr.contains("decoder_full_layer_session")
            && stderr.contains("private"),
        "fixture must fail specifically because the real decoder full layer authority field is private:\n{stderr}"
    );
    assert!(
